  ...
  return (
    <div className="canvas">
      <Canvas camera={{ position: [4, 3, 6], fov: 45 }}>
        ...
        <rectAreaLight position={[0, 4, 1]} width={5} height={3} intensity={8} />
        <Room />
        {items.map((item) => {
          const selected = item.id === selectedId;
          const project = ready.get(item.projectId);
          return project ? (
            <TransformControls key={item.id} enabled={selected} mode="translate">
              <group
              ...
              >
                <Gltf src={`/api/proxy/projects/${project.id}/export`} />
              </group>
            </TransformControls>
          ) : null;
        })}
